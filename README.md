...
`database/gooq/cmd/ggen` generates typed table objects from database schemas:

```bash
cd database/gooq/cmd/ggen && go run . gen dao -link "mysql:root:pass@tcp(127.0.0.1:3306)/db" -t user -tablePath table
```

- Template-driven (`template/gooq_table.tpl`, overridable via `-t5` external path).
...
- Default drivers: sqlite/mssql/clickhouse (no facade dependency); MySQL/PG enabled by uncommenting the import.

## Testing

```bash
go test ./database/gooq/ -count=1              # main library (rendering assertions)
cd database/gooq/e2e && go test . -count=1     # e2e against a real MySQL
cd database/gooq/cmd/ggen && go test ./...     # ggen end-to-end
```
