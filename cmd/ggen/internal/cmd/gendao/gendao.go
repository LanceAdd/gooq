...
type (
	CGenDao      struct{}
	CGenDaoInput struct {
		gmeta.Meta         `name:"dao" config:"{CGenDaoConfig}" usage:"{CGenDaoUsage}" brief:"{CGenDaoBrief}" eg:"{CGenDaoEg}" ad:"{CGenDaoAd}"`
...
func (c CGenDao) Dao(ctx context.Context, in CGenDaoInput) (out *CGenDaoOutput, err error) {
	...
	} else if gcfg.Instance().Available(ctx) {
		v := gcfg.Instance().MustGet(ctx, CGenDaoConfig)
		if v.IsSlice() {
...
func doGenDaoForArray(ctx context.Context, index int, in CGenDaoInput) {
	...
	if in.Link != "" {
		...
		if db, err = gdb.Instance(in.Group); err != nil {
			mlog.Fatalf(`database initialization failed: %+v`, err)
		}
	}
